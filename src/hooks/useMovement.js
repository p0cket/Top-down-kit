...
const useMovement = (
  ...
  const checkCollisions = (newX, newY) => {
    const playerRect = createRect({ x: newX, y: newY })

    if (doesItCollide(playerRect, walls)) return true
    if (doesItCollide(playerRect, npcs, () => onCollision(newX, newY)))
      return true
    if (doesItCollide(playerRect, enemies)) return true
  ...
  const moveCharacter = () => {
    ...
    if (keysPressed.current["ArrowUp"] || keysPressed.current["w"]) {
      if (!checkCollisions(x, y - step)) y -= step
    }
    if (keysPressed.current["ArrowDown"] || keysPressed.current["s"]) {
      if (!checkCollisions(x, y + step)) y += step
    }
    if (keysPressed.current["ArrowLeft"] || keysPressed.current["a"]) {
      if (!checkCollisions(x - step, y)) x -= step
    }
